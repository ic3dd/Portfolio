...
const placeholderFeatureKeys: Record<string, TranslationKey[]> = {
  "1": [
    "projects.item1.f1",
    "projects.item1.f2",
    "projects.item1.f3",
    ...
    <section id="projetos" className="scroll-mt-20 border-t border-border px-4 py-16 sm:px-6 lg:px-8 lg:py-20">
      <div className="mx-auto max-w-6xl">
        ...
        <ul className="mt-10 grid grid-cols-1 gap-8 lg:grid-cols-2">
          {list.map((project, i) => {
            const mappedTitle =
              isPlaceholder && project.id === "1"
                ? t("projects.item1.title")
                : isPlaceholder && project.id === "2"
                  ? t("projects.item2.title")
                  : project.title;
            const mappedDesc =
              isPlaceholder && project.id === "1"
                ? t("projects.item1.desc")
                : isPlaceholder && project.id === "2"
                  ? t("projects.item2.desc")
                  : project.description;
            const demoBadge =
              isPlaceholder && project.id === "1"
                ? t("projects.item1.badge")
                : isPlaceholder && project.id === "2"
                  ? t("projects.item2.badge")
                  : null;
            const featureKeys = isPlaceholder ? placeholderFeatureKeys[project.id] : undefined;
